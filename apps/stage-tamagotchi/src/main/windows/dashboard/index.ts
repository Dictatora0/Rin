...
export async function setupDashboardWindow(params: {
  ...
  const windowConfig = getConfig().windows?.find(w => (w.title === 'Rin Dashboard' || w.title === 'AIRI Dashboard') && w.tag === 'dashboard')
  ...
  function handleNewBounds(newBounds: Rectangle) {
    ...
    const existingConfigIndex = config.windows.findIndex(w => (w.title === 'Rin Dashboard' || w.title === 'AIRI Dashboard') && w.tag === 'dashboard')
    ...
    if (existingConfigIndex === -1) {
      ...
      const windowConfig = defu(config.windows[existingConfigIndex], { title: 'Rin Dashboard', tag: 'dashboard' })
