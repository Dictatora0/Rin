...
var ModuleFactory = (() => {
  ...
  return async function (moduleArg = {}) {
    ...
    // Memory management
    let /** @type {!Int8Array} */ HEAP8, /** @type {!Uint8Array} */ HEAPU8, /** @type {!Int16Array} */ HEAP16, /** @type {!Uint16Array} */ HEAPU16, /** @type {!Int32Array} */ HEAP32, /** @type {!Uint32Array} */ HEAPU32, /** @type {!Float32Array} */ HEAPF32, /** @type {!Float64Array} */ HEAPF64
    ...
    /** @param {boolean|number=} implicit */ const exitJS = (status, implicit) => {
      EXITSTATUS = status
      _proc_exit(status)
    }
    ...
    /** @param {number=} timeout */ const safeSetTimeout = (func, timeout) => setTimeout(() => {
    ...
    var Browser = {
      ...
      calculateMouseCoords(pageX, pageY) {
        ...
        let adjustedX = pageX - (scrollX + rect.left)
        let adjustedY = pageY - (scrollY + rect.top)
    ...
    var PATH = {
      ...
      normalize: (path) => {
        const isAbsolute = PATH.isAbs(path); const trailingSlash = path.slice(-1) === '/'
        // Normalize the path
    ...
    var PATH_FS = {
      resolve: (...args) => {
        let resolvedPath = ''; let resolvedAbsolute = false
    ...
    /** @type {function(string, boolean=, number=)} */ const intArrayFromString = (stringy, dontAddNull, length) => {
    ...
    var FS = {
      ...
      unmount(mountpoint) {
        ...
        for (let [hash, current] of Object.entries(FS.nameTable)) {
          while (current) {
      ...
      createDefaultDevices() {
        ...
        const randomBuffer = new Uint8Array(1024); let randomLeft = 0
        const randomByte = () => {
          if (randomLeft === 0) {
    ...
    const BindingError = class BindingError extends Error {
      constructor(message) {
        super(message)
    ...
    /** @param {object=} options */ function sharedRegisterType(rawType, registeredInstance, options = {}) {
      ...
      if (Object.hasOwn(registeredTypes, rawType)) {
      ...
      if (Object.hasOwn(awaitingDependencies, rawType)) {
    ...
    const emval_handles = [0, 1, , 1, null, 1, true, 1, false, 1]
    ...
    const UTF16ToString = (ptr, maxBytesToRead, ignoreNul) => {
      const idx = ((ptr) >> 1)
    ...
    const UTF32ToString = (ptr, maxBytesToRead, ignoreNul) => {
      let str = ''
      const startIdx = ((ptr) >> 2)
      // If maxBytesToRead is not passed explicitly, it will be undefined, and this
      // will always evaluate to true. This saves on code size.
      for (let i = 0; !(i >= maxBytesToRead / 4); i++) {
        const utf32 = HEAPU32[startIdx + i]
        if (!utf32 && !ignoreNul)
          break
    ...
    const __embind_register_std_wstring = (rawType, charSize, name) => {
      name = AsciiToString(name)
      let decodeString, encodeString, lengthBytesUTF
      if (charSize === 2) {
      ...
      registerType(rawType, {
        ...
        fromWireType: (value) => {
          ...
          const length = HEAPU32[((value) >> 2)]
          const str = decodeString(value + 4, length * charSize, true)
          _free(value)
          return str
        },
        toWireType: (destructors, value) => {
    ...
    const createNamedFunction = (name, func) => Object.defineProperty(func, 'name', {
      value: name,
    })

    const emval_returnValue = (toReturnWire, destructorsRef, handle) => {
      const destructors = []
      const result = toReturnWire(destructors, handle)
    ...
    const __emval_create_invoker = (argCount, argTypesPtr, kind) => {
      ...
      const [retType, ...argTypes] = emval_lookupTypes(argCount, argTypesPtr)
      const toReturnWire = retType.toWireType.bind(retType)
      const argFromPtr = argTypes.map(type => type.readValueFromPointer.bind(type))
      argCount--
      // remove the extracted return type
      const argN = new Array(argCount)
      const invokerFunction = (handle, methodName, destructorsRef, args) => {
        let offset = 0
        for (let i = 0; i < argCount; ++i) {
      ...
      const functionName = `methodCaller<(${argTypes.map(t => t.name)}) => ${retType.name}>`
    ...
    const __emval_instanceof = (object, constructor) => {
      object = Emval.toValue(object)
      constructor = Emval.toValue(constructor)
      return object instanceof constructor
    }

    const __emval_invoke = (caller, handle, methodName, destructorsRef, args) => emval_methodCallers[caller](handle, methodName, destructorsRef, args)

    const __emval_new_cstring = v => Emval.toHandle(getStringOrSymbol(v))
    ...
    const __emval_set_property = (handle, key, value) => {
      handle = Emval.toValue(handle)
      key = Emval.toValue(key)
    ...
    function _clock_time_get(clk_id, ignored_precision_low, ignored_precision_high, ptime) {
      const ignored_precision = convertI32PairToI53Checked(ignored_precision_low, ignored_precision_high)
    ...
    const _emscripten_asm_const_ptr = (code, sigPtr, argbuf) => runEmAsmFunction(code, sigPtr, argbuf)

    const _emscripten_errn = (str, len) => err(UTF8ToString(str, len))

    const getHeapMax = () => // Stay one Wasm page short of 4GB: while e.g. Chrome is able to allocate
    ...
    const _emscripten_has_asyncify = () => 0

    const _emscripten_outn = (str, len) => out(UTF8ToString(str, len))
    ...
    /** @returns {number} */ const convertFrameToPC = (frame) => {
      let match
    ...
    const saveInUnwindCache = (callstack) => {
      for (const line of callstack) {
        const pc = convertFrameToPC(line)
        if (pc) {
    ...
    const jsStackTrace = () => (new Error()).stack.toString()

    const _emscripten_stack_snapshot = () => {
    ...
    const _emscripten_pc_get_function = (pc) => {
    ...
    const _emscripten_stack_unwind_buffer = (addr, buffer, count) => {
      let stack
    ...
    const webgl_enable_WEBGL_draw_instanced_base_vertex_base_instance = ctx => // Closure is expected to be allowed to minify the '.dibvbi' property, so not accessing it quoted.
      !!(ctx.dibvbi = ctx.getExtension('WEBGL_draw_instanced_base_vertex_base_instance'))

    const webgl_enable_WEBGL_multi_draw_instanced_base_vertex_base_instance = ctx => !!(ctx.mdibvbi = ctx.getExtension('WEBGL_multi_draw_instanced_base_vertex_base_instance'))

    const webgl_enable_EXT_polygon_offset_clamp = ctx => !!(ctx.extPolygonOffsetClamp = ctx.getExtension('EXT_polygon_offset_clamp'))

    const webgl_enable_EXT_clip_control = ctx => !!(ctx.extClipControl = ctx.getExtension('EXT_clip_control'))

    const webgl_enable_WEBGL_polygon_mode = ctx => !!(ctx.webglPolygonMode = ctx.getExtension('WEBGL_polygon_mode'))

    const webgl_enable_WEBGL_multi_draw = ctx => // Closure is expected to be allowed to minify the '.multiDrawWebgl' property, so not accessing it quoted.
      !!(ctx.multiDrawWebgl = ctx.getExtension('WEBGL_multi_draw'))

    const getEmscriptenSupportedExtensions = (ctx) => {
      // Restrict the list of advertised extensions to those that we actually
      // support.
      const supportedExtensions = [ // WebGL 1 extensions
    ...
    var GL = {
      ...
      createContext: (/** @type {HTMLCanvasElement} */ canvas, webGLContextAttributes) => {
        ...
        const ctx = (webGLContextAttributes.majorVersion > 1) ? canvas.getContext('webgl2', webGLContextAttributes) : canvas.getContext('webgl', webGLContextAttributes)
        if (!ctx)
          return 0
    ...
    /** @type {object} */ const specialHTMLTargets = [0, globalThis.document ?? 0, globalThis.window ?? 0]
    ...
    const findEventTarget = (target) => {
      ...
      if (target === '#window')
        return window; else if (target === '#document')
        return document; else if (target === '#screen')
        return screen; else if (target === '#canvas')
        return Module.canvas; else if (typeof target == 'string')
        return globalThis.document?.getElementById(target)
      return target
    }

    const findCanvasEventTarget = (target) => {
      if (typeof target == 'number')
    ...
    const _emscripten_webgl_get_context_attributes = (c, a) => {
      if (!a)
        return -5
      c = GL.contexts[c]
      if (!c)
        return -3
      const t = c.GLctx?.getContextAttributes()
      if (!t)
        return -3
      HEAP8[a] = t.alpha
      HEAP8[(a) + (1)] = t.depth
      ...
      const power = t.powerPreference && webglPowerPreferences.indexOf(t.powerPreference)
      HEAP32[(((a) + (8)) >> 2)] = power
    ...
    const _emscripten_webgl_do_get_current_context = () => GL.currentContext ? GL.currentContext.handle : 0
    ...
    const readI53FromI64 = ptr => HEAPU32[((ptr) >> 2)] + HEAP32[(((ptr) + (4)) >> 2)] * 4294967296
    ...
    var WebGPU = {
      ...
      importJsAdapter: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateAdapter(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsBindGroup: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateBindGroup(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsBindGroupLayout: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateBindGroupLayout(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsBuffer: (buffer, parentPtr = 0) => {
        ...
        const bufferPtr = _emwgpuImportBuffer(parentPtr)
        WebGPU.Internals.jsObjectInsert(bufferPtr, buffer)
        return bufferPtr
      },
      importJsCommandBuffer: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateCommandBuffer(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsCommandEncoder: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateCommandEncoder(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsComputePassEncoder: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateComputePassEncoder(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsComputePipeline: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateComputePipeline(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsDevice: (device, parentPtr = 0) => {
        const queuePtr = _emwgpuCreateQueue(parentPtr)
        const devicePtr = _emwgpuCreateDevice(parentPtr, queuePtr)
      ...
      importJsExternalTexture: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateExternalTexture(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsPipelineLayout: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreatePipelineLayout(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsQuerySet: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateQuerySet(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsQueue: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateQueue(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsRenderBundle: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateRenderBundle(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsRenderBundleEncoder: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateRenderBundleEncoder(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsRenderPassEncoder: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateRenderPassEncoder(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsRenderPipeline: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateRenderPipeline(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsSampler: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateSampler(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsShaderModule: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateShaderModule(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsSurface: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateSurface(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsTexture: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateTexture(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      importJsTextureView: (obj, parentPtr = 0) => {
        const ptr = _emwgpuCreateTextureView(parentPtr)
        WebGPU.Internals.jsObjects[ptr] = obj
        return ptr
      },
      ...
      iterateExtensions: (root, handlers) => {
        for (let ptr = HEAPU32[((root) >> 2)]; ptr; ptr = HEAPU32[((ptr) >> 2)]) {
          const sType = HEAP32[(((ptr) + (4)) >> 2)]
          // This will crash if there's no handler indicating either a bogus
          // sType, or one we haven't implemented yet.
          const handler = handlers[sType](ptr)
        }
      },
      ...
      makeTexelCopyBufferLayout: (ptr) => {
        const bytesPerRow = HEAPU32[(((ptr) + (8)) >> 2)]
        const rowsPerImage = HEAPU32[(((ptr) + (12)) >> 2)]
        return {
      ...
      makeTexelCopyBufferInfo: (ptr) => {
        const layoutPtr = ptr + 0
        const bufferCopyView = WebGPU.makeTexelCopyBufferLayout(layoutPtr)
      ...
      makePipelineConstants: (constantCount, constantsPtr) => {
        ...
        for (let i = 0; i < constantCount; ++i) {
          const entryPtr = constantsPtr + 24 * i
          const key = WebGPU.makeStringFromStringView(entryPtr + 4)
      ...
      makeRenderPipelineDesc: (descriptor) => {
        ...
        function makeColorState(csPtr) {
          const format = WebGPU.TextureFormat[HEAP32[(((csPtr) + (4)) >> 2)]]
          return format
            ? {
                format,
        ...
        function makeVertexBuffer(vbPtr) {
          if (!vbPtr)
            return undefined
          const stepMode = WebGPU.VertexStepMode[HEAP32[(((vbPtr) + (4)) >> 2)]]
          const attributeCount = HEAPU32[(((vbPtr) + (16)) >> 2)]
      ...
      fillLimitStruct: (limits, limitsOutPtr) => {
        const nextInChainPtr = HEAPU32[((limitsOutPtr) >> 2)]
        function setLimitValueU32(name, basePtr, limitOffset, fallbackValue = 0) {
          const limitValue = limits[name] ?? fallbackValue
        ...
        if (nextInChainPtr !== 0) {
          const sType = HEAP32[(((nextInChainPtr) + (4)) >> 2)]
          const compatibilityModeLimitsPtr = nextInChainPtr
      ...
      fillAdapterInfoStruct: (info, infoStruct) => {
        ...
        const strs = info.vendor + info.architecture + info.device + info.description
        let strPtr = stringToNewUTF8(strs)
        const vendorLen = lengthBytesUTF8(info.vendor)
        WebGPU.setStringView(infoStruct + 4, strPtr, vendorLen)
        strPtr += vendorLen
        const architectureLen = lengthBytesUTF8(info.architecture)
        WebGPU.setStringView(infoStruct + 12, strPtr, architectureLen)
        strPtr += architectureLen
        const deviceLen = lengthBytesUTF8(info.device)
        WebGPU.setStringView(infoStruct + 20, strPtr, deviceLen)
        strPtr += deviceLen
        const descriptionLen = lengthBytesUTF8(info.description)
        WebGPU.setStringView(infoStruct + 28, strPtr, descriptionLen)
        strPtr += descriptionLen
        HEAP32[(((infoStruct) + (36)) >> 2)] = 2
        const adapterType = info.isFallbackAdapter ? 3 : 4
    ...
    const _emscripten_webgpu_get_device = () => {
    ...
    const _emwgpuBufferDestroy = (bufferPtr) => {
      const buffer = WebGPU.getJsObject(bufferPtr)
      const onUnmap = WebGPU.Internals.bufferOnUnmaps[bufferPtr]
      if (onUnmap) {
        for (let i = 0; i < onUnmap.length; ++i) {
          onUnmap[i]()
        }
    ...
    const _emwgpuBufferGetMappedRange = (bufferPtr, offset, size) => {
      const buffer = WebGPU.getJsObject(bufferPtr)
      if (size == -1)
        size = undefined
      let mapped
    ...
    const _emwgpuBufferUnmap = (bufferPtr) => {
      const buffer = WebGPU.getJsObject(bufferPtr)
      const onUnmap = WebGPU.Internals.bufferOnUnmaps[bufferPtr]
      if (!onUnmap) {
        // Already unmapped
    ...
    const _emwgpuDelete = (ptr) => {
      delete WebGPU.Internals.jsObjects[ptr]
    }

    const _emwgpuDeviceCreateBuffer = (devicePtr, descriptor, bufferPtr) => {
      const mappedAtCreation = !!(HEAPU32[(((descriptor) + (32)) >> 2)])
      const desc = {
        label: WebGPU.makeStringFromOptionalStringView(descriptor + 4),
    ...
    const _emwgpuDeviceCreateComputePipelineAsync = function (devicePtr, futureId_low, futureId_high, descriptor, pipelinePtr) {
      const futureId = convertI32PairToI53Checked(futureId_low, futureId_high)
      const desc = WebGPU.makeComputePipelineDesc(descriptor)
      const device = WebGPU.getJsObject(devicePtr)
      ...
      WebGPU.Internals.futureInsert(futureId, device.createComputePipelineAsync(desc).then((pipeline) => {
        ...
        callUserCallback(() => {
          ...
          const status = pipelineError.reason === 'validation' ? 3 : pipelineError.reason === 'internal' ? 4 : 0
    ...
    const _emwgpuDeviceCreateRenderPipelineAsync = function (devicePtr, futureId_low, futureId_high, descriptor, pipelinePtr) {
      const futureId = convertI32PairToI53Checked(futureId_low, futureId_high)
      const desc = WebGPU.makeRenderPipelineDesc(descriptor)
      const device = WebGPU.getJsObject(devicePtr)
    ...
    const _emwgpuDeviceCreateShaderModule = (devicePtr, descriptor, shaderModulePtr) => {
    ...
    const _emwgpuDeviceDestroy = (devicePtr) => {
      const device = WebGPU.getJsObject(devicePtr)
    ...
    const _emwgpuWaitAny = (futurePtr, futureCount, timeoutMSPtr) => {
    ...
    /** @param {number=} offset */ const doReadv = (stream, iov, iovcnt, offset) => {
      let ret = 0
    ...
    /** @param {number=} offset */ const doWritev = (stream, iov, iovcnt, offset) => {
      let ret = 0
      for (let i = 0; i < iovcnt; i++) {
        const ptr = HEAPU32[((iov) >> 2)]
        const len = HEAPU32[(((iov) + (4)) >> 2)]
    ...
    const _emscripten_glActiveTexture = x0 => GLctx.activeTexture(x0)

    const _glActiveTexture = _emscripten_glActiveTexture
    ...
    const _emscripten_glBindBufferBase = (target, index, buffer) => {
      GLctx.bindBufferBase(target, index, GL.buffers[buffer])
    }

    const _glBindBufferBase = _emscripten_glBindBufferBase

    const _emscripten_glBindFramebuffer = (target, framebuffer) => {
      GLctx.bindFramebuffer(target, GL.framebuffers[framebuffer])
    }

    const _glBindFramebuffer = _emscripten_glBindFramebuffer
    ...
    const _emscripten_glBindVertexArray = (vao) => {
      GLctx.bindVertexArray(GL.vaos[vao])
      const ibo = GLctx.getParameter(34965)
      GLctx.currentElementArrayBufferBinding = ibo ? (ibo.name | 0) : 0
    }

    const _glBindVertexArray = _emscripten_glBindVertexArray

    const _emscripten_glBlendEquation = x0 => GLctx.blendEquation(x0)

    const _glBlendEquation = _emscripten_glBlendEquation
    ...
    const _emscripten_glClearColor = (x0, x1, x2, x3) => GLctx.clearColor(x0, x1, x2, x3)

    const _glClearColor = _emscripten_glClearColor

    const convertI32PairToI53 = (lo, hi) => (lo >>> 0) + hi * 4294967296

    const _emscripten_glClientWaitSync = (sync, flags, timeout_low, timeout_high) => {
    ...
    const _glClientWaitSync = _emscripten_glClientWaitSync
    ...
    const _emscripten_glDeleteFramebuffers = (n, framebuffers) => {
      for (let i = 0; i < n; ++i) {
    ...
    const _glDeleteFramebuffers = _emscripten_glDeleteFramebuffers
    ...
    const _emscripten_glDeleteSync = (id) => {
      if (!id)
        return
    ...
    const _glDeleteSync = _emscripten_glDeleteSync
    ...
    const _emscripten_glDeleteVertexArrays = (n, vaos) => {
      for (let i = 0; i < n; i++) {
    ...
    const _glDeleteVertexArrays = _emscripten_glDeleteVertexArrays
    ...
    const _emscripten_glDrawBuffers = (n, bufs) => {
    ...
    const _glDrawBuffers = _emscripten_glDrawBuffers
    ...
    const _emscripten_glFenceSync = (condition, flags) => {
    ...
    const _glFenceSync = _emscripten_glFenceSync

    const _emscripten_glFinish = () => GLctx.finish()

    const _glFinish = _emscripten_glFinish

    const _emscripten_glFlush = () => GLctx.flush()

    const _glFlush = _emscripten_glFlush

    const _emscripten_glFramebufferTexture2D = (target, attachment, textarget, texture, level) => {
      GLctx.framebufferTexture2D(target, attachment, textarget, GL.textures[texture], level)
    }

    const _glFramebufferTexture2D = _emscripten_glFramebufferTexture2D

    const _emscripten_glFramebufferTextureLayer = (target, attachment, texture, level, layer) => {
      GLctx.framebufferTextureLayer(target, attachment, GL.textures[texture], level, layer)
    }

    const _glFramebufferTextureLayer = _emscripten_glFramebufferTextureLayer
    ...
    const _emscripten_glGenFramebuffers = (n, ids) => {
      GL.genObject(n, ids, 'createFramebuffer', GL.framebuffers)
    }

    const _glGenFramebuffers = _emscripten_glGenFramebuffers
    ...
    const _emscripten_glGenVertexArrays = (n, arrays) => {
      GL.genObject(n, arrays, 'createVertexArray', GL.vaos)
    }

    const _glGenVertexArrays = _emscripten_glGenVertexArrays

    const _emscripten_glGetAttribLocation = (program, name) => GLctx.getAttribLocation(GL.programs[program], UTF8ToString(name))

    const _glGetAttribLocation = _emscripten_glGetAttribLocation

    const _emscripten_glGetError = () => {
      const error = GLctx.getError() || GL.lastError
      GL.lastError = 0
      return error
    }

    const _glGetError = _emscripten_glGetError
      ...
      if (ret === undefined) {
        ...
        switch (typeof result) {
          ...
          case 'object':
            ...
            else if (result instanceof Float32Array || result instanceof Uint32Array || result instanceof Int32Array || Array.isArray(result)) {
    ...
    const _emscripten_glGetFloatv = (name_, p) => emscriptenWebGLGet(name_, p, 2)

    const _glGetFloatv = _emscripten_glGetFloatv
    ...
    const _emscripten_glGetProgramiv = (program, pname, p) => {
      ...
      if (pname == 35716) {
        ...
        if (!program.maxUniformLength) {
          const numActiveUniforms = GLctx.getProgramParameter(program, 35718)
        ...
        if (!program.maxAttributeLength) {
          const numActiveAttributes = GLctx.getProgramParameter(program, 35721)
        ...
        if (!program.maxUniformBlockNameLength) {
          const numActiveUniformBlocks = GLctx.getProgramParameter(program, 35382)
      ...
      if (!ret) {
        switch (name_) {
          ...
          case 35724:
            ...
            var ver_re = /^WebGL GLSL ES (\d\.\d\d?)(?:$| .*)/
    ...
    const _emscripten_glGetUniformBlockIndex = (program, uniformBlockName) => GLctx.getUniformBlockIndex(GL.programs[program], UTF8ToString(uniformBlockName))

    const _glGetUniformBlockIndex = _emscripten_glGetUniformBlockIndex

    /** @suppress {checkTypes} */ const jstoi_q = str => Number.parseInt(str)

    /** @noinline */ const webglGetLeftBracePos = name => name.slice(-1) == ']' && name.lastIndexOf('[')
    ...
    const _emscripten_glLineWidth = x0 => GLctx.lineWidth(x0)

    const _glLineWidth = _emscripten_glLineWidth
    ...
    const _emscripten_glPixelStorei = (pname, param) => {
      if (pname == 3317) {
    ...
    const _glPixelStorei = _emscripten_glPixelStorei
    ...
    const _emscripten_glReadPixels = (x, y, width, height, format, type, pixels) => {
      if (GL.currentContext.version >= 2) {
    ...
    const _glReadPixels = _emscripten_glReadPixels
    ...
    const _emscripten_glTexImage2D = (target, level, internalFormat, width, height, border, format, type, pixels) => {
      if (GL.currentContext.version >= 2) {
    ...
    const _emscripten_glTexParameterf = (x0, x1, x2) => GLctx.texParameterf(x0, x1, x2)

    const _glTexParameterf = _emscripten_glTexParameterf

    const _emscripten_glTexParameterfv = (target, pname, params) => {
      const param = HEAPF32[((params) >> 2)]
      GLctx.texParameterf(target, pname, param)
    }

    const _glTexParameterfv = _emscripten_glTexParameterfv

    const _emscripten_glTexParameteri = (x0, x1, x2) => GLctx.texParameteri(x0, x1, x2)

    const _glTexParameteri = _emscripten_glTexParameteri

    const _emscripten_glTexStorage2D = (x0, x1, x2, x3, x4) => GLctx.texStorage2D(x0, x1, x2, x3, x4)

    const _glTexStorage2D = _emscripten_glTexStorage2D

    const _emscripten_glTexStorage3D = (x0, x1, x2, x3, x4, x5) => GLctx.texStorage3D(x0, x1, x2, x3, x4, x5)

    const _glTexStorage3D = _emscripten_glTexStorage3D

    const _emscripten_glTexSubImage2D = (target, level, xoffset, yoffset, width, height, format, type, pixels) => {
      if (GL.currentContext.version >= 2) {
        if (GLctx.currentPixelUnpackBufferBinding) {
      ...
      const pixelData = pixels ? emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, 0) : null
      GLctx.texSubImage2D(target, level, xoffset, yoffset, width, height, format, type, pixelData)
    }

    const _glTexSubImage2D = _emscripten_glTexSubImage2D

    const _emscripten_glTexSubImage3D = (target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, pixels) => {
      if (GLctx.currentPixelUnpackBufferBinding) {
    ...
    const _glTexSubImage3D = _emscripten_glTexSubImage3D
    ...
    const _emscripten_glUniform1fv = (location, count, value) => {
      if (GL.currentContext.version >= 2) {
    ...
    const _glUniform1fv = _emscripten_glUniform1fv

    const _emscripten_glUniform1i = (location, v0) => {
      GLctx.uniform1i(webglGetUniformLocation(location), v0)
    }

    const _glUniform1i = _emscripten_glUniform1i
    ...
    const _emscripten_glUniform2fv = (location, count, value) => {
      if (GL.currentContext.version >= 2) {
    ...
    const _glUniform2fv = _emscripten_glUniform2fv
    ...
    const _emscripten_glUniform4f = (location, v0, v1, v2, v3) => {
      GLctx.uniform4f(webglGetUniformLocation(location), v0, v1, v2, v3)
    }

    const _glUniform4f = _emscripten_glUniform4f

    const _emscripten_glUniform4fv = (location, count, value) => {
      if (GL.currentContext.version >= 2) {
    ...
    const _glUniform4fv = _emscripten_glUniform4fv

    const miniTempWebGLIntBuffers = []

    const _emscripten_glUniform4iv = (location, count, value) => {
      if (GL.currentContext.version >= 2) {
    ...
    const _glUniform4iv = _emscripten_glUniform4iv

    const _emscripten_glUniformBlockBinding = (program, uniformBlockIndex, uniformBlockBinding) => {
      program = GL.programs[program]
      GLctx.uniformBlockBinding(program, uniformBlockIndex, uniformBlockBinding)
    }

    const _glUniformBlockBinding = _emscripten_glUniformBlockBinding

    const _emscripten_glUniformMatrix2fv = (location, count, transpose, value) => {
      if (GL.currentContext.version >= 2) {
    ...
    const _glUniformMatrix2fv = _emscripten_glUniformMatrix2fv

    const _emscripten_glUniformMatrix3fv = (location, count, transpose, value) => {
      if (GL.currentContext.version >= 2) {
    ...
    const _glUniformMatrix3fv = _emscripten_glUniformMatrix3fv
    ...
    const _emscripten_glVertexAttribPointer = (index, size, type, normalized, stride, ptr) => {
      const cb = GL.currentContext.clientBuffers[index]
    ...
    const _emscripten_glViewport = (x0, x1, x2, x3) => GLctx.viewport(x0, x1, x2, x3)
    ...
    const _wgpuCommandEncoderBeginComputePass = (encoderPtr, descriptor) => {
      let desc
      if (descriptor) {
        desc = {
          label: WebGPU.makeStringFromOptionalStringView(descriptor + 4),
      ...
      const commandEncoder = WebGPU.getJsObject(encoderPtr)
      const ptr = _emwgpuCreateComputePassEncoder(0)
      WebGPU.Internals.jsObjectInsert(ptr, commandEncoder.beginComputePass(desc))
      return ptr
    }

    const _wgpuCommandEncoderBeginRenderPass = (encoderPtr, descriptor) => {
      function makeColorAttachment(caPtr) {
        const viewPtr = HEAPU32[(((caPtr) + (4)) >> 2)]
        if (viewPtr === 0) {
          // Null `view` means no attachment in this slot.
          return undefined
        }
        let depthSlice = HEAPU32[(((caPtr) + (8)) >> 2)]
        if (depthSlice == 4294967295)
      ...
      function makeRenderPassDescriptor(descriptor) {
        ...
        if (nextInChainPtr !== 0) {
          const sType = HEAP32[(((nextInChainPtr) + (4)) >> 2)]
          const renderPassMaxDrawCount = nextInChainPtr
      ...
      const commandEncoder = WebGPU.getJsObject(encoderPtr)
      const ptr = _emwgpuCreateRenderPassEncoder(0)
      WebGPU.Internals.jsObjectInsert(ptr, commandEncoder.beginRenderPass(desc))
      return ptr
    }

    const _wgpuCommandEncoderCopyBufferToTexture = (encoderPtr, srcPtr, dstPtr, copySizePtr) => {
      const commandEncoder = WebGPU.getJsObject(encoderPtr)
    ...
    const _wgpuCommandEncoderCopyTextureToBuffer = (encoderPtr, srcPtr, dstPtr, copySizePtr) => {
      const commandEncoder = WebGPU.getJsObject(encoderPtr)
      const copySize = WebGPU.makeExtent3D(copySizePtr)
      commandEncoder.copyTextureToBuffer(WebGPU.makeTexelCopyTextureInfo(srcPtr), WebGPU.makeTexelCopyBufferInfo(dstPtr), copySize)
    }

    const _wgpuCommandEncoderCopyTextureToTexture = (encoderPtr, srcPtr, dstPtr, copySizePtr) => {
      const commandEncoder = WebGPU.getJsObject(encoderPtr)
      const copySize = WebGPU.makeExtent3D(copySizePtr)
    ...
    const _wgpuCommandEncoderFinish = (encoderPtr, descriptor) => {
      // TODO: Use the descriptor.
      const commandEncoder = WebGPU.getJsObject(encoderPtr)
      const ptr = _emwgpuCreateCommandBuffer(0)
      WebGPU.Internals.jsObjectInsert(ptr, commandEncoder.finish())
      return ptr
    }

    const _wgpuComputePassEncoderDispatchWorkgroups = (passPtr, x, y, z) => {
      const pass = WebGPU.getJsObject(passPtr)
      pass.dispatchWorkgroups(x, y, z)
    }

    const _wgpuComputePassEncoderEnd = (passPtr) => {
      const pass = WebGPU.getJsObject(passPtr)
      pass.end()
    }

    const _wgpuComputePassEncoderSetBindGroup = (passPtr, groupIndex, groupPtr, dynamicOffsetCount, dynamicOffsetsPtr) => {
      const pass = WebGPU.getJsObject(passPtr)
      const group = WebGPU.getJsObject(groupPtr)
    ...
    const _wgpuComputePassEncoderSetPipeline = (passPtr, pipelinePtr) => {
      const pass = WebGPU.getJsObject(passPtr)
      const pipeline = WebGPU.getJsObject(pipelinePtr)
      pass.setPipeline(pipeline)
    }

    const _wgpuComputePipelineGetBindGroupLayout = (pipelinePtr, groupIndex) => {
      const pipeline = WebGPU.getJsObject(pipelinePtr)
      const ptr = _emwgpuCreateBindGroupLayout(0)
      WebGPU.Internals.jsObjectInsert(ptr, pipeline.getBindGroupLayout(groupIndex))
      return ptr
    }

    const _wgpuDeviceCreateBindGroup = (devicePtr, descriptor) => {
      function makeEntry(entryPtr) {
        const bufferPtr = HEAPU32[(((entryPtr) + (8)) >> 2)]
        const samplerPtr = HEAPU32[(((entryPtr) + (32)) >> 2)]
        const textureViewPtr = HEAPU32[(((entryPtr) + (36)) >> 2)]
        let externalTexturePtr = 0
        ...
        if (bufferPtr) {
          ...
          let size = readI53FromI64((entryPtr) + (24))
          if (size == -1)
            size = undefined
          resource = {
      ...
      const device = WebGPU.getJsObject(devicePtr)
      const ptr = _emwgpuCreateBindGroup(0)
      WebGPU.Internals.jsObjectInsert(ptr, device.createBindGroup(desc))
      return ptr
    }

    const _wgpuDeviceCreateBindGroupLayout = (devicePtr, descriptor) => {
      function makeBufferEntry(substructPtr) {
        const typeInt = HEAPU32[(((substructPtr) + (4)) >> 2)]
        if (!typeInt)
          return undefined
        return {
      ...
      function makeTextureEntry(substructPtr) {
        const sampleTypeInt = HEAPU32[(((substructPtr) + (4)) >> 2)]
        if (!sampleTypeInt)
          return undefined
        return {
      ...
      function makeStorageTextureEntry(substructPtr) {
        const accessInt = HEAPU32[(((substructPtr) + (4)) >> 2)]
        if (!accessInt)
          return undefined
        return {
    ...
    const _wgpuDeviceCreateCommandEncoder = (devicePtr, descriptor) => {
      let desc
      if (descriptor) {
        desc = {
          label: WebGPU.makeStringFromOptionalStringView(descriptor + 4),
      ...
      const device = WebGPU.getJsObject(devicePtr)
      const ptr = _emwgpuCreateCommandEncoder(0)
      WebGPU.Internals.jsObjectInsert(ptr, device.createCommandEncoder(desc))
      return ptr
    }

    const _wgpuDeviceCreateComputePipeline = (devicePtr, descriptor) => {
      const desc = WebGPU.makeComputePipelineDesc(descriptor)
      const device = WebGPU.getJsObject(devicePtr)
      const ptr = _emwgpuCreateComputePipeline(0)
      WebGPU.Internals.jsObjectInsert(ptr, device.createComputePipeline(desc))
      return ptr
    }

    const _wgpuDeviceCreatePipelineLayout = (devicePtr, descriptor) => {
      const bglCount = HEAPU32[(((descriptor) + (12)) >> 2)]
      const bglPtr = HEAPU32[(((descriptor) + (16)) >> 2)]
      const bgls = []
    ...
    const _wgpuDeviceCreateRenderPipeline = (devicePtr, descriptor) => {
      const desc = WebGPU.makeRenderPipelineDesc(descriptor)
      const device = WebGPU.getJsObject(devicePtr)
      const ptr = _emwgpuCreateRenderPipeline(0)
      WebGPU.Internals.jsObjectInsert(ptr, device.createRenderPipeline(desc))
      return ptr
    }

    const _wgpuDeviceCreateSampler = (devicePtr, descriptor) => {
      let desc
      if (descriptor) {
        desc = {
          label: WebGPU.makeStringFromOptionalStringView(descriptor + 4),
      ...
      const device = WebGPU.getJsObject(devicePtr)
      const ptr = _emwgpuCreateSampler(0)
      WebGPU.Internals.jsObjectInsert(ptr, device.createSampler(desc))
      return ptr
    }

    const _wgpuDeviceCreateTexture = (devicePtr, descriptor) => {
      const nextInChainPtr = HEAPU32[((descriptor) >> 2)]
      let textureBindingViewDimension
      if (nextInChainPtr !== 0) {
        const sType = HEAP32[(((nextInChainPtr) + (4)) >> 2)]
        const textureBindingViewDimensionDescriptor = nextInChainPtr
      ...
      const viewFormatCount = HEAPU32[(((descriptor) + (52)) >> 2)]
      if (viewFormatCount) {
        const viewFormatsPtr = HEAPU32[(((descriptor) + (56)) >> 2)]
      ...
      const device = WebGPU.getJsObject(devicePtr)
      const ptr = _emwgpuCreateTexture(0)
      WebGPU.Internals.jsObjectInsert(ptr, device.createTexture(desc))
      return ptr
    }

    const _wgpuQueueSubmit = (queuePtr, commandCount, commands) => {
      const queue = WebGPU.getJsObject(queuePtr)
      const cmds = Array.from(HEAP32.subarray((((commands) >> 2)), ((commands + commandCount * 4) >> 2)), id => WebGPU.getJsObject(id))
      queue.submit(cmds)
    }

    function _wgpuQueueWriteBuffer(queuePtr, bufferPtr, bufferOffset_low, bufferOffset_high, data, size) {
      const bufferOffset = convertI32PairToI53Checked(bufferOffset_low, bufferOffset_high)
      const queue = WebGPU.getJsObject(queuePtr)
      const buffer = WebGPU.getJsObject(bufferPtr)
    ...
    const _wgpuQueueWriteTexture = (queuePtr, destinationPtr, data, dataSize, dataLayoutPtr, writeSizePtr) => {
      const queue = WebGPU.getJsObject(queuePtr)
      const destination = WebGPU.makeTexelCopyTextureInfo(destinationPtr)
      const dataLayout = WebGPU.makeTexelCopyBufferLayout(dataLayoutPtr)
    ...
    const _wgpuRenderPassEncoderDraw = (passPtr, vertexCount, instanceCount, firstVertex, firstInstance) => {
      firstVertex >>>= 0
      firstInstance >>>= 0
      const pass = WebGPU.getJsObject(passPtr)
    ...
    const _wgpuRenderPassEncoderEnd = (encoderPtr) => {
      const encoder = WebGPU.getJsObject(encoderPtr)
      encoder.end()
    }

    const _wgpuRenderPassEncoderSetBindGroup = (passPtr, groupIndex, groupPtr, dynamicOffsetCount, dynamicOffsetsPtr) => {
      const pass = WebGPU.getJsObject(passPtr)
      const group = WebGPU.getJsObject(groupPtr)
      if (dynamicOffsetCount == 0) {
        pass.setBindGroup(groupIndex, group)
    ...
    const _wgpuRenderPassEncoderSetPipeline = (passPtr, pipelinePtr) => {
      const pass = WebGPU.getJsObject(passPtr)
      const pipeline = WebGPU.getJsObject(pipelinePtr)
      pass.setPipeline(pipeline)
    }

    const _wgpuRenderPipelineGetBindGroupLayout = (pipelinePtr, groupIndex) => {
      const pipeline = WebGPU.getJsObject(pipelinePtr)
      const ptr = _emwgpuCreateBindGroupLayout(0)
      WebGPU.Internals.jsObjectInsert(ptr, pipeline.getBindGroupLayout(groupIndex))
      return ptr
    ...
    const _wgpuTextureCreateView = (texturePtr, descriptor) => {
      let desc
      if (descriptor) {
        let swizzle
        const nextInChainPtr = HEAPU32[((descriptor) >> 2)]
        if (nextInChainPtr !== 0) {
          const sType = HEAP32[(((nextInChainPtr) + (4)) >> 2)]
          const swizzleDescriptor = nextInChainPtr
          const swizzlePtr = swizzleDescriptor + 8
          const r = WebGPU.ComponentSwizzle[HEAP32[((swizzlePtr) >> 2)]] || 'r'
          const g = WebGPU.ComponentSwizzle[HEAP32[(((swizzlePtr) + (4)) >> 2)]] || 'g'
          const b = WebGPU.ComponentSwizzle[HEAP32[(((swizzlePtr) + (8)) >> 2)]] || 'b'
          const a = WebGPU.ComponentSwizzle[HEAP32[(((swizzlePtr) + (12)) >> 2)]] || 'a'
          swizzle = `${r}${g}${b}${a}`
        }
        const mipLevelCount = HEAPU32[(((descriptor) + (24)) >> 2)]
        const arrayLayerCount = HEAPU32[(((descriptor) + (32)) >> 2)]
        desc = {
          label: WebGPU.makeStringFromOptionalStringView(descriptor + 4),
      ...
      const texture = WebGPU.getJsObject(texturePtr)
      const ptr = _emwgpuCreateTextureView(0)
      WebGPU.Internals.jsObjectInsert(ptr, texture.createView(desc))
      return ptr
    }

    const _wgpuTextureDestroy = (texturePtr) => {
      WebGPU.getJsObject(texturePtr).destroy()
    }

    const _wgpuTextureGetFormat = (texturePtr) => {
      const texture = WebGPU.getJsObject(texturePtr)
