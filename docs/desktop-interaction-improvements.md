...
- 本轮仅做学习统计图表的视觉层优化，不新增/删除图表，不改学习 store 和导出逻辑。
- 六个既有图表（趋势、7天柱状、热力图、任务完成结构、优先级分布、专注质量）统一为同一套卡片视觉语言：
  - 统一卡片圆角、边框、背景与间距节奏
  - 统一标题/副标题/图例/空状态样式
  - 统一关键数值文本表达，避免只依赖颜色传达信息
- 图表配色收敛到语义变量（primary/success/warning/danger/muted/surface/border），减少组件内零散硬编码颜色。
- 趋势图保留 SVG 轻量实现，增强了面积渐变、主线可读性、最高点/今日点识别和累计摘要。
- 任务完成结构图保留环形图实现，空任务时显示明确空状态，避免误导性 0% 大圆环。
- 专注质量改为统一 KPI 小卡样式；优先级分布改为统一语义色条；热力图补齐“少/多”图例和格子可访问文本。
- 设置页中的“学习统计图表”分区继续按“趋势分析 / 任务分析 / 质量与历史”组织，宽屏双列、窄屏单列，不影响导出区域交互。
